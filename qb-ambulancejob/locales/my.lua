local Translations = {
    error = {
        canceled = 'Batal',
        bled_out = 'Anda luka berdarah...',
        impossible = 'Tindakan Mustahil...',
        no_player = 'Tiada Orang Berhampiran',
        no_firstaid = 'Anda Perlukan First Aid Kit',
        no_bandage = 'Anda Perlukan Bandage',
        beds_taken = 'Katil telah digunakan...',
        possessions_taken = 'Semua harta kamu telah disita...',
        not_enough_money = 'Anda tidak mempunyai wang mencukupi...',
        cant_help = 'You can\'t help this person...',
        not_ems = 'Anda bukan KK atau belum log masuk',
        not_online = 'Pemain Tidak Bernyawa'
    },
    success = {
        revived = 'Anda Menyelamatkan Pesakit',
        healthy_player = 'Pesakit Telah Dipulihkan',
        helped_player = 'Anda Bantu Pesakit Pulih',
        wounds_healed = 'Luka Anda Telah Dipulihkan!',
        being_helped = 'Anda Sedang Di Rawat...'
    },
    info = {
        civ_died = 'Orang Awam Meninggal',
        civ_down = 'Orang Awam Kecundang',
        civ_call = 'Orang Awam Hubunggi',
        self_death = 'Bunuh Diri',
        wep_unknown = 'Tidak tahu',
        respawn_txt = 'RESPAWN IN: ~r~%{deathtime}~s~ SECONDS',
        respawn_revive = 'TEKAN [~r~E~s~] UNTUK %{holdtime} SAAT UNTUK HIDUP BAYARAN $~r~%{cost}~s~',
        bleed_out = 'YOU WILL BLEED OUT IN: ~r~%{time}~s~ SECONDS',
        bleed_out_help = 'YOU WILL BLEED OUT IN: ~r~%{time}~s~ SECONDS, YOU CAN BE HELPED',
        request_help = 'TEKAN [~r~G~s~] MINTA BANTUAN',
        help_requested = 'K KESIHATAN TELAH DI HUBUNGGI',
        amb_plate = 'KKEM', -- Should only be 4 characters long due to the last 4 being a random 4 digits
        heli_plate = 'KKER',  -- Should only be 4 characters long due to the last 4 being a random 4 digits
        status = 'Semak Status',
        is_staus = 'Is %{status}',
        healthy = 'Anda telah di pulihkan!',
        safe = 'Hospital Selamat',
        pb_hospital = 'Hospital Kementerian Kesihatan',
        pain_message = 'Anda %{limb} Keadaan %{severity}',
        many_places = 'You have pain in many places...',
        bleed_alert = 'You are %{bleedstate}',
        ems_alert = 'KK Alert - %{text}',
        mr = 'En.',
        mrs = 'Pn.',
        dr_needed = 'A doctor is needed at Pillbox Hospital',
        ems_report = 'KK Laporan',
        message_sent = 'Message to be sent',
        check_health = 'Check a Players Health',
        heal_player = 'Sembuhkan Pesakit',
        revive_player = 'Senyawakan Pesakit',
        revive_player_a = 'Revive A Player or Yourself (Admin Only)',
        player_id = 'Player ID (may be empty)',
        pain_level = 'Set Yours or A Players Pain Level (Admin Only)',
        kill = 'Kill A Player or Yourself (Admin Only)',
        heal_player_a = 'Heal A Player or Yourself (Admin Only)',
    },
    mail = {
        sender = 'Kementerian Kesihatan',
        subject = 'Kos Hospital',
        message = 'Dear %{gender} %{lastname}, <br /><br />Hereby you received an email with the costs of the last hospital visit.<br />The final costs have become: <strong>$%{costs}</strong><br /><br />We wish you a quick recovery!'
    },
    states = {
        irritated = 'irritated',
        quite_painful = 'quite painful',
        painful = 'painful',
        really_painful = 'really painful',
        little_bleed = 'bleeding a little bit...',
        bleed = 'bleeding...',
        lot_bleed = 'bleeding a lot...',
        big_bleed = 'bleeding very much...',
    },
    menu = {
        amb_vehicles = 'Ambulance Vehicles',
        status = 'Health Status',
        close = '⬅ Close Menu',
    },
    text = {
        pstash_button = '[E] - Personal stash',
        pstash = 'Personal stash',
        onduty_button = '[E] - Go On Duty',
        offduty_button = '[E] - Go Off Duty',
        duty = 'On/Off Duty',
        armory_button = '[E] - Armory',
        armory = 'Armory',
        veh_button = '[E] - Grab / Store Vehicle',
        heli_button = '[E] - Grab / Store Helicopter',
        elevator_roof = '[E] - Take the elevator to the roof',
        elevator_main = '[E] - Take the elevator down',
        bed_out = '[E] - To get out of bed..',
        call_doc = '[E] - Call doctor',
        call = 'Call',
        check_in = '[E] Check in',
        check = 'Check In',
        lie_bed = '[E] - To lie in bed'
    },
    body = {
        head = 'Head',
        neck = 'Neck',
        spine = 'Spine',
        upper_body = 'Upper Body',
        lower_body = 'Lower Body',
        left_arm = 'Left Arm',
        left_hand = 'Left Hand',
        left_fingers = 'Left Fingers',
        left_leg = 'Left Leg',
        left_foot = 'Left Foot',
        right_arm = 'Right Arm',
        right_hand = 'Right Hand',
        right_fingers = 'Right Fingers',
        right_leg = 'Right Leg',
        right_foot = 'Right Foot',
    },
    progress = {
        ifaks = 'Taking ifaks...',
        bandage = 'Using Bandage...',
        painkillers = 'Taking Painkillers...',
        revive = 'Reviving Person...',
        healing = 'Healing Wounds...',
        checking_in = 'Checking in...',
    },
    logs = {
        death_log_title = "%{playername} (%{playerid}) is dead",
        death_log_message = "%{killername} has killed %{playername} with a **%{weaponlabel}** (%{weaponname})",
    }
}

Lang = Lang or Locale:new({
    phrases = Translations,
    warnOnMissing = true
})

